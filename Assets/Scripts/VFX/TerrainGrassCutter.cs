...
public class TerrainGrassCutter : MonoBehaviour
{
    ...
    private Vector3 GetCutCenterOnTerrain(Vector2 facingDir, float forwardDistance)
    {
        ...
        float terrainHeight = targetTerrain.SampleHeight(cutCenter) + terrainPosition.y;
    ...
    private GrassCutResult BuildResult(
        ...
        bool grassRemoved)
    {
        if (!grassRemoved || removedDetailCount <= 0 || targetTerrain == null)
        {
        ...
        float worldY = targetTerrain.SampleHeight(new Vector3(worldX, terrainPosition.y, worldZ)) + terrainPosition.y;
